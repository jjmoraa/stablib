...
from stablib.models.model5DOFs import mass, damping, stiffness, viscous_damper
...
for iom, omega in enumerate(omegas): #rads
    ...
    zeta_t1=0.2
    zeta_t2=0.1
    Ct = lambda t: viscous_damper(omega, t, [2*zeta_edge*edgNatFreq_rad*m, 2*zeta_edge*edgNatFreq_rad*m, 2*zeta_edge*edgNatFreq_rad*m, 2*zeta_t1*np.sqrt(kx*M), 2*zeta_t2*np.sqrt(ky*M)])
...
for iom, omega in enumerate(omegas): #rads
    ...
    for iind in range(len(unique_indices)):
        eigenvalues_unique [iom, iind, :] = eigenvalues_exp_corrected_for_range[iom, 0, :] + 1j * np.tile(unique_indices[None, :] * omega, (10, 1))[:,iind]
...
plotCampbellDiagramAllModesSingleHarmonic(vf_0_sorted[:,:,0:(relevant_modes)],freqs_Hz,0,var_name='Frequency [Hz]',save_path=None)
plotCampbellDiagramAllModesSingleHarmonic(zeta_for_range_sorted[:,:,0:(relevant_modes)],freqs_Hz,0,var_name='Damping ratio',save_path=None)

# All modes, 1st harmonic (1)
plotCampbellDiagramAllModesSingleHarmonic(vf_0_sorted[:,:,0:(relevant_modes)],freqs_Hz,1,var_name='Frequency [Hz]',save_path=None)
plotCampbellDiagramAllModesSingleHarmonic(zeta_for_range_sorted[:,:,0:(relevant_modes)],freqs_Hz,1,var_name='Damping ratio',save_path=None)
#Fan of modes
mod_indices = [0, 1, 3]
