...
arrays_by_op, A_interp, u_vel, omegas, T_rotor = openfast.openFAST_A_interpreter(foldername)
...
for iom, omega in enumerate(omegas): #rads
    ...
    C = np.eye(16)
    #C[0, 0] = 1.0
    #C[1, 2] = 1.0 is this part really necessary. Discussion with RIVA at some point?
...
plotCampbellDiagramAllModesSingleHarmonic(vf_0_sorted[:,:,:],u_vel,0,var_name='Velocity [m/s]',save_path=None)
plotCampbellDiagramAllModesSingleHarmonic(zeta_for_range_sorted[:,:,0:(relevant_modes)],u_vel,0,var_name='Velocity [m/s]',save_path=None)
