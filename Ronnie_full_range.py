...
for iom, omega in enumerate(omegas):
    ...
    sol = solve_ivp(ode_system, [0, 2*np.pi], x0, t_eval=time, method='RK45')
    ...
    [D,V] = np.linalg.eig(B) #get floquet eigenvalues and eigenvectors
    ...
    y0 = q_values[0] * np.array([np.eye(problem_size)])  # Initial condition
    ...
    omega_n = np.sqrt(np.real(D)**2+np.imag(D)**2)
    fd = np.imag(D)/(2*np.pi) # damped frequencies in Hz
    fn = omega_n/(2*np.pi) # natural frequencies
    print('f_n')
    print(fn)
    zeta = -np.real(D)/omega_n  # damping ratio
    #print('fn:', fn.sort()[0:3])
    #print('ze:', fn.sort()[0:3])
    eigenvalues_for_range.append(D)
    mode_frequencies.append(fd)
    mode_decay.append(np.real(D))
...
cols_multipliers = ['Omega'] + [f'multipliers({i+1})' for i in range(multipliers_for_range.shape[1] - 1)]
